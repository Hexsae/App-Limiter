...
class Main_UI(QMainWindow):
    def __init__(self):
        ...
        self.setWindowIcon(QIcon("okhand.png"))
        self.finished = []
        ...
        self.helpbtn = self.findChild(QAction, "helpbtn")
        ...
        self.helpbtn.triggered.connect(self.helpbtn_clicked)
    ...
    def handle_progress(self):
        try:
            ...
            processes = list(set(map(lambda x: x.name().capitalize(), proc_list)))
            ...
            for item in self.data:
                ...
                if not self.data[item]["disabled"] and not item in self.finished:
                ...
                if self.data[item]["time"] == 0  and not item in self.finished:
                    self.finished.append(item)
                    box = QMessageBox.warning(self, "Time up!",
                    "You have used {} for the set amount of time!".format(item, time), QMessageBox.Ok)
...
class AddWindow(QMainWindow):
    ...
    def load_processes(self):
        ...
        processes = [x.capitalize() for x in processes]
        processes.insert(0, "")
